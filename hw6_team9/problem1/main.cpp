...
{ // Pricing European Put Option
    ...
    int M = 4; double alpha = 0.45;
    FDM_OP option;
    option.SetOption(S0, K, T, v, q, r, AmerEuro, PutCall);
    tuple<vector<vector<double>>, vector<double>, vector<double>> output = option.FDM(alpha, M);
    ...
    cout << "\nM error_pointwise Ratio(error_pointwise) error_pointwise_2 Ratio(error_pointwise2) error_RMS Ratio(error_RMS) Delta Gamma Theta" << endl;
    vector<double> statistics_old, statistics_new, greeks;
    for (int M=4; M<=256; M*=4)
    {
        output = option.FDM(alpha, M);
        ...
        if (M==4)
        {
            cout << statistics_new[0] << ", , " << statistics_new[1] << ", , " << statistics_new[2] << ", , "
                 << greeks[0] << " " << greeks[1] << " " << greeks[2] << endl;
        }
        else
        {
            cout << statistics_new[0] << ", " << statistics_new[0]/statistics_old[0] << ", "
                 << statistics_new[1] << ", " << statistics_new[1]/statistics_old[1] << ", "
                 << statistics_new[2] << ", " << statistics_new[2]/statistics_old[2] << ", "
                 << greeks[0] << ", " << greeks[1] << ", " << greeks[2] << endl;
        }
