...
std::tuple<double,int> Secant(double x0, double x1, std::function<double(double)> fn)
{//Use Secant's method to solve f(x)=0
	double tol_approx = 0.00001;	//Secant method stopped at (1) |f(x_new)|<=tol_approx
	double tol_consec = 0.00001;	//Secant method stopped at (1) |x_new-x_old|<=tol_consec
	...
	return std::make_tuple(x_new,iter);
}
