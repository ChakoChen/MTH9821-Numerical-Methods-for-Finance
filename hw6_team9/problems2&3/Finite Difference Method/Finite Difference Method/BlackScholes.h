...
{
	double d1 = (std::log(op.S0 / op.K) + (op.r - op.q + op.sigma*op.sigma / 2.0)*(op.T)) / (op.sigma*std::sqrt(op.T));
	...
	{//put option
		...
		//gamma
		result.push_back((std::exp(-op.q*op.T)) / (op.S0*op.sigma*std::sqrt(op.T))*(1) / (std::sqrt(2 * pi))*std::exp(-d1*d1 / 2.0));
		//theta
		result.push_back(-(op.S0*op.sigma*std::exp(-op.q*op.T)) / (2 * std::sqrt(2 * pi*op.T))*std::exp(-d1*d1 / 2.0) - op.q*op.S0*std::exp(-op.q*op.T)*normalDistribution(-d1) + op.r*op.K*std::exp(-op.r*op.T)*normalDistribution(-d2));
		//vega
...
double BSImpliedVol(const Option& op, const double & ini1, const double & ini2, const double& marketp, const char & pc)
{
	...
	if (pc == 'p')
	{
		impvol = std::get<0>(Secant(ini1, ini2, psolver));
	}
	else if (pc == 'c')
	{
		impvol = std::get<0>(Secant(ini1, ini2, csolver));
